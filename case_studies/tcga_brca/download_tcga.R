...
dir_create(OUT_DIR, recursive = TRUE)
...
mirna_se <- GDCprepare(mirna_q)

mirna_mat <- log1p(assay(mirna_se, "reads_per_million_miRNA_mapped"))
...
# Handle both SummarizedExperiment and data.frame return types
if (is(protein_data, "SummarizedExperiment")) {
    protein_mat <- assay(protein_data)
} else {
    # data.frame: first column = antibody names, remaining numeric columns = samples
...
# Map GDC primary_diagnosis to histological_type
htype <- ifelse(
